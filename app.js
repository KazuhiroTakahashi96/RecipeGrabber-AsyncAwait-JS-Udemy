...
function displayRecipes(recipeResults) {
  ...
  recipeResults.forEach((recipeResults) => {
    recipeEl += `
    <div class="item">
        <img src="${recipeResults.recipe.image}" />
        <div class="content-wrapper">
            <h2 class="recipe-title">${recipeResults.recipe.label}</h2>
            <a href="${
              recipeResults.recipe.url
            }" target="_blank" class="view-recipe">View Recipe</a>
        </div>
        <div class="recipe-desc">
            <p class="item-data">Calories: ${recipeResults.recipe.calories.toFixed(
              2
            )}</p>
            <p class="item-data">Diet Label: ${
              recipeResults.recipe.dietLabels
            }</p>
            <p class="item-data">Health Label: ${
              recipeResults.recipe.healthLabels
            }</p>
            <p class="item-data">Source: ${recipeResults.recipe.source}</p>
        </div>
  </div>`;
